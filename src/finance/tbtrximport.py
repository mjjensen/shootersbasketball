...
# encoding: utf-8
'''
tbtrximport -- import Trybooking "Account Transactions" CSV report file(s)

import Trybooking "Account Transactions" CSV report file(s)
--
@version:    0.1
@created:    2016-04-29
    ...
    def main(self):
        ...
        try:
            ...
            for csvfile in self.args.csvlist:
                csvrecords.extend(
                    read_csv(csvfile, tb_trx_csvinfo, self.args.verbose)
                )
        ...
        try:
            ...
            for csvrecord in sorted(csvrecords, key=lambda r: r.date):
                ...
                maybe_dups = db.trybooking_query.filter(
                    db.Trybooking.type == csvrecord.type.csv_value,
                        ...
                        if found_dup:
                            raise RuntimeError(
                                'matched more than one! {} and {}'.format(
                                    trybooking_format(prev_dup),
                                    trybooking_format(maybe_dup)
                                )
                            )
